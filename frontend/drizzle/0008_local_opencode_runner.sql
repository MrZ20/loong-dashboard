...
CREATE TABLE IF NOT EXISTS opencode_session_bindings (
  ...
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL,
  UNIQUE(user_id, session_scope, commit_sha),
  FOREIGN KEY(user_id) REFERENCES users(id)
);

ALTER TABLE analysis_documents ADD COLUMN opencode_session_id TEXT;
...
ALTER TABLE chat_threads ADD COLUMN opencode_session_id TEXT;
ALTER TABLE chat_threads ADD COLUMN opencode_commit_sha TEXT NOT NULL DEFAULT '';
...
CREATE INDEX IF NOT EXISTS idx_opencode_binding_scope
  ON opencode_session_bindings(user_id, session_scope, commit_sha);
